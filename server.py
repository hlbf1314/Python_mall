...
# insert into shop values("qwe","qwe","qwe","123","北京市","2022-12-15 22:46:10")
# alter table `shop` convert to character set utf8;
class Shop:
    ...
    def addgoods(self, recv):
        op1 = Shop_op()
        if op1.add_goods(recv['user'], recv['goods_name'], recv['goods_type'], recv['goods_prices'],
                         recv['goods_rest']):
            data = {'result': 'success'}
        else:
            data = {'result': 'fail'}
    ...
    def updategoods(self, recv):
        op1 = Shop_op()
        if op1.update_goods(recv['user'], recv['old_goods_name'], recv['goods_name'], recv['goods_type'],
                            recv['goods_prices'], recv['goods_rest']):
            data = {'result': 'success'}
        else:
            data = {'result': 'fail'}
        self.conn.send(json.dumps(data).encode())
